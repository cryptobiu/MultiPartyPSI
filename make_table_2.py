...
with open("{0}/experiment2_avg.csv".format(dir_name), 'rb') as csvfile:
    ...
    with open("{0}/experiment2.txt".format(dir_name), 'wb') as f:
        ...
        f.write('\\end{tabular}\n')
        f.write('\\caption{Runtimes in seconds for PSI protocols for $\sigma=64$, set size $2^{16}$, $\symsec=80$, infinite number of threads and 5 parties in different network scenarios}\n')
