...
void SimpleHashingNaiveFollower::run() {
    ...
    for (uint32_t k=0; k < m_followerSet.m_numOfBins; k++) {
        ...
        uint32_t numOfRandomBytes = (m_maxBinSize-numOfElementsInBin)*m_followerSet.m_maskSizeInBytes;
        ...
        send_masks(random.get(), m_maxBinSize-numOfElementsInBin, m_followerSet.m_maskSizeInBytes, m_leader);
