...
void PsiParty::initializeMaskSize() {
    switch(m_strategy) {
        ...
        case Strategy::SIMPLE_HASH:
            m_maskbitlen = pad_to_multiple(m_parameters.m_statSecParameter + (m_numOfParties-1)*ceil_log2(NUM_HASH_FUNCTIONS*m_maxBinSize), 8);
            break;
...
uint32_t PsiParty::runAsLeader() {
    ...
    hash_table.reset(cuckoo_hashing(m_eleptr.get(), m_setSize, m_numOfBins, hs,
                                nelesinbin.get(), perm.get(), m_numCores, bin_ids.get(), hashed_by.get()));
