...
void SimpleHashingNaiveFollower::run() {
    ...
    for (uint32_t k=0; k < m_followerSet.m_numOfBins; k++) {
        ...
        uint32_t numOfRandomBytes = (m_followerSet.m_numOfHashFunctions*m_followerSet.m_maxBinSize-numOfElementsInBin)*m_followerSet.m_maskSizeInBytes;
        ...
        if (numOfRandomBytes != 0) {
            ...
            send_masks(random.data(), m_followerSet.m_numOfHashFunctions*m_followerSet.m_maxBinSize-numOfElementsInBin, m_followerSet.m_maskSizeInBytes, m_leader);
        }
