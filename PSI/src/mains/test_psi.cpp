...
int32_t main(int32_t argc, char** argv) {
	...
	for(uint32_t i = 0; i < nruns; i++) {
		if(role == LEADER) cout << "Running test on iteration " << i << std::flush;
		nelements = rand() % (1<<12);
		elebytelen = (rand() % 12) + 4;

		test_psi_prot(role, sockfd.data(), nelements, elebytelen, crypt);
		if(role == LEADER) cout << endl;
	}

	if(role == LEADER) cout << "All tests successfully passed" << endl;
}
...
uint32_t test_psi_prot(role_type role, CSocket* sock, uint32_t nelements,
	...
	if(role == LEADER) cout << " for |A|=" << nelements << ", |B|=" << pnelements << ", b=" << elebytelen << ": " << std::flush;
